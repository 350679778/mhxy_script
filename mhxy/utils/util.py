...
def locate_center_on_screen(frame:Frame, pic, confidence=0.9):
    confidence = confidence if __open_cv_enable() else None
    if isinstance(pic, list):
        location = None
        for i in pic:
            if confidence is not None:
                location = pyautogui.locateCenterOnScreen(i, region=(frame.left, frame.top, frame.right, frame.bottom),
        ...
        if confidence is not None:
            return pyautogui.locateCenterOnScreen(pic, region=(frame.left, frame.top, frame.right - frame.left, frame.top - frame.bottom),
                                                  confidence=confidence)
        else:
